...
export function useModelConfig() {
  ...
  const availableModels = computed(() =>
    modelSource.value === 'online' ? onlineModels.value : localModels.value
  )

  const modelStatusClass = computed(() => {
    if (modelStatus.value.includes('成功') || modelStatus.value.includes('可用') || modelStatus.value.includes('发现')) {
